...
function App() {
  ...
  const handleAdd = (user) => {
    if(member.includes(user.id)) {
      setAlert({message:"Already in the list", variant:'danger', id : user.id});
    } else {
      const newList = [...member, user.id];
      setMember(newList);
      setAlert({message: 'Added Successfully', variant:'success', id : user.id});
  ...
  const handleDelete = (user) => {
    if(member.includes(user.id)) {
      ...
      setMember(newList);
      setAlert({message: 'Deleted Successfully', variant:'success', id: user.id});
      const newAllSalary = [parseInt(allSalary) - parseInt(user.annual_salary)];
      setAllSalary(newAllSalary);

    } else {
      setAlert({message:'Member does not exist', variant:'danger', id: user.id});
    }
    window.setTimeout(()=>{
      setAlert({visible:false})
    },2000)
